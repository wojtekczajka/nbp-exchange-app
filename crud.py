...
def get_nearest_currency_rate(db: Session, table: str, currency: str, date_obj: datetime.date):
    if table in ['A', 'B']:
        result = db.query(CurrencyAB).filter(CurrencyAB.table == table, CurrencyAB.currency ==
                                             currency, CurrencyAB.date <= date_obj).order_by(desc(CurrencyAB.date)).first()
        if not result:
            result = db.query(CurrencyAB).filter(CurrencyAB.table == table, CurrencyAB.currency ==
                                                 currency, CurrencyAB.date >= date_obj).order_by(asc(CurrencyAB.date)).first()
    elif table == 'C':
        result = db.query(CurrencyC).filter(CurrencyC.table == table, CurrencyC.currency ==
                                            currency, CurrencyC.date <= date_obj).order_by(desc(CurrencyC.date)).first()
        if not result:
            result = db.query(CurrencyC).filter(CurrencyC.table == table, CurrencyC.currency ==
                                                currency, CurrencyC.date >= date_obj).order_by(asc(CurrencyC.date)).first()
    return result
